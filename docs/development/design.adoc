...
Auf dem Terminal läuft eine minimale REST-API, welche RFID-Ausweise, Piloten und Einstellungen entgegennimmt. Im Falle, dass ein Pilot synchronisiert wird, prüft das Terminal, ob dieser bereits existiert. Ist dies der Fall, wird der bestehende Pilot aktualisiert, andernfalls wird er neu eingefügt. Geplant war noch eine CORS-Policy, die nur Anfragen vom Webserver zulässt, allerdings war dies nicht mehr komplett zeitlich umsetzbar. Für jede Flugsession gibt es ein Synced Flag, das angibt, ob die aktuelle Version der Flugsession bereits mit dem Webserver synchronisiert wurde. Sobald sich etwas an einer Flugsession ver;ndert, etwa wenn der Flugleiterstatus gesetzt wird, werden alle Sessions mit ungesetztem Flag synchronisiert, wenn sie die Toleranzzeit nicht unterschreiten. Die Toleranzzeit gibt dabei an, wie lange nach der Erstellung eine Session frühestens synchronisert werden darf. Dies ist wichtig, damit Anmeldungen, die aus Versehen passiert sind, nicht auf dem Webserver erscheinen.

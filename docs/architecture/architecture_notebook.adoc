...
Die logische Sicht enthält alle architektur-relevanten Klassen des Systems und wie diese in Beziehung zueinander stehen. Man kann hier unter anderem erkennen, dass der Pilot das Zentrum der Sicht markiert. Durch das Terminal wird Flugsession erzeugt, welche die Beziehung zwischen Pilot und Protokoll herstellt.

.Domainmodell
:imagesdir: {docs-requirements}/images/Domainmodell
image::domain_model.png[Domainmodell, width=70%]
...
Aus physischer Sicht ist eine klare Zweiteilung der Funktionalitäten erkennbar. Diese Zweiteilung zwischen Terminal und Webserver existiert auch physisch. Man kann außerdem gut erkennen, über welche Komponenten Informationen in das System gelangen und ausgegeben werden.

.C4-Modell (Level 2)
:imagesdir: {docs-architecture}/images/C4_Model
image::C4_Level2.png[C4-Modell (Level 2), width=70%]

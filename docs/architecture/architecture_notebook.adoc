...
* Im Gespräch mit den Themenstellern hat sich ergeben, dass es jeweils ein Zugang für die Rollen Administrator und einen Zugang für die Rolle Pilot geben soll. Jeder Pilot loggt sich dann also über den Piloten-Zugang ein mit beschränkter Funktionalität und jeder Admin über den Admin-Zugang mit vollem Funktionsumfang ein. Dadurch wird ein versehentlicher Eingriff in den systemkritischen Bereich umgangen.
* Die konkrete technische Realisierung dieser getrennten Logins steht noch nicht fest.
...
* Zunächst steht die Ausgestaltung des Frontends für den Desktop-PC zu Testzwecken im Vordergrund
...
* die triviale Aktualisierung der Uhrzeit des Raspis (zeitgesetuert)
...
* Die Schaffung der Schnittstellen streben wir über Flask-Server an, die über REST API miteinander kommunizieren. Wir haben bereits mit einem Backend-Prototypen des Webservers erste Requests realisiert.
* Auf Seite des Terminals haben wir in der Tabelle Flugsession ein Synced Flag hinzugefügt, welches angibt, ob die aktuelle Version einer Session bereits mit dem Webserver synchronisiert wurde. Nach jeder Änderung einer Session wird das Flag auf 0 gesetzt und alle nicht synchronisierten Sessions werden per POST-Request an den Webserver gesendet. Weiterhin haben wir auf dem Terminal eine minimale REST-API implementiert, die POST-Requests zur Synchronisierung von RFID-Ausweisen, Piloten und Einstellungen empfängt.
...
* Es steht noch aus, ob die Konfiguration sich ausschließlich um die Zeitdauern dreht, oder ob auch weitere Parameter (LED-Farbe, LED-Blink-Rhythmus, Befehlsreihenfolge) konfigurierbar sein sollen.
...
* Es wird sich beim Bau eines Prototypen des Terminals zeigen, wie der Umgang mit den RFID-Ausweisen zu implementieren ist

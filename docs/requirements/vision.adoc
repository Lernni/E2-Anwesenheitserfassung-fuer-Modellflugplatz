...
|eine erfolgreiche Lösung wäre | Ein digitales System für die Anwesenheitserfassung bzw. Protokollierung der Piloten über ein Terminal, der die An- und Abmeldedaten einliest, verarbeitet und sicher über einen Webserver einsehbar und verwaltbar macht.
|===
...
* problematisch dabei ist, das es schwierig unter verschluss zu halten ist, um es nur autorisierten Mitgliedern zugänglich zu machen

.Zukunftige Systemplattform:
* zukünftig soll ein digitales System das physische System ersetzten
* es soll über einen Raspberry Pi Daten einlesen, verarbeiten und sicher über einen Webserver einsehbar und verwaltbar machen

==== zu integrierende Anwendungen:

// auch Anbindung an Webserver
* in einem vergangenden Projekt (SE-Projekt: "Wetterstation") enstand eine wetterfeste Infrastruktur, die eine Anbindung eines RFID-Readers ermöglichen sollte
...
** es gibt ca. ... Admins 
** Anzahl der Admins ändert sich in laufe der Zeit selten
* aus rechlichen Gründen, muss die Anwesenheit der Piloten erfasst & protokolliert werden
...
* sorgfälltig und behutsam mit diesen Rechten umgehen 
...
//Wie oft kommen neu-Anmeldungen vor, Wie oft Gäste 
** parallel nutzen 5-10 Piloten den Flugplatz gleichzeitig (das fliegen von Modellflugzeugen) 
* aus rechlichen Gründen haben die Piloten beim betreten des Flugplatzes die Aufgabe, sich "anzumelden"
...
** darf auf Anfrage bei den Administratoren in seine persöhnlichen Daten einsehen 

...
// tag::nfaf2[]
* *NFAF-2*: Datensynchronisierung (ereignisgesteuert): Protokoll- und Pilotendaten werden bei der Anmeldung direkt an den Web-Server gesendet
// end::nfaf2[]

* *NFAS-3*: Überwachung der Synchronisation: Der Administrator kann einsehen, wann das Flugprotokoll zuletzt synchronisiert wurde und so Rückschlüsse auf mögliche Kommunikationsprobleme ziehen
...
// tag::nfa+3[]
* *NFA+-3*: Ressourcenbegrenzungen: Stromversorgung des Raspberry Pi durch Solarkollektoren
** Auf stromsparende Arbeitsweise achten
// end::nfa+3[]

// tag::nfa+4[]
//* *NFA+-4*: Physische Begrenzung: bevorzugt robustes Design des Terminals (E/A-Kompomenten in einem Gehäuse verpackt, Raspi im Boden verbaut)
// -> schränkt den Entwurf des Systems nicht ein
// end::nfa+4[]
